...
def apply_calibrated_threshold(
    probabilities: np.ndarray,
    threshold_chlorella: float
) -> int:
    """
...
def validate_submission(
    ...
    with open(submission_path, 'r') as f:
        ...
        header = next(reader)
        if header != ['ID', 'TARGET']:
            issues.append(f"Invalid header: {header}. Expected: ['ID', 'TARGET']")
        ...
        for row_num, row in enumerate(reader, start=2):
            ...
            if subject_id in ids_seen:
                issues.append(f"Row {row_num}: Duplicate ID: {subject_id}")
            ...
            try:
                ...
                if target_int < 0 or target_int > 4:
                    issues.append(f"Row {row_num}: TARGET {target_int} out of range [0, 4]")
            except ValueError:
                issues.append(f"Row {row_num}: TARGET '{target}' is not an integer")
        
        # Check row count if expected provided
        if expected_count is not None and row_count != expected_count:
            issues.append(f"Expected {expected_count} predictions, got {row_count}")
