...
def plot_pr_curves(
    ...
    for class_id in range(5):
        ...
        if class_id == 0 and highlight_chlorella:
            plt.plot(recall, precision, 'r-', linewidth=2.5, 
                    label=f'{CLASS_ID_TO_NAME[class_id]} (priority)', zorder=10)
        else:
            plt.plot(recall, precision, linewidth=1.5, 
                    label=CLASS_ID_TO_NAME[class_id], alpha=0.7)
