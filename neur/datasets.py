...
def get_train_transforms(
    ...
    return A.Compose(
        [
            ...
            A.GaussianBlur(
                blur_limit=(3, 7), sigma_limit=(blur_sigma_min, blur_sigma_max), p=blur_prob
            ),
        ],
...
class SubjectDataset(Dataset):
    ...
    def __init__(
        self, subjects: Dict[str, Dict], transform: Optional[Callable] = None, img_size: int = 224
    ):
        """
        Initialize dataset.
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int]:
        ...
        mask_indicator = np.array(
            [float(amp_present), float(phase_present), float(mask_present)], dtype=np.float32
        ).mean()  # Average presence indicator

        # Broadcast mask indicator to full spatial dimensions
        mask_indicator_ch = np.full(
            (1, img_3ch.shape[1], img_3ch.shape[2]), mask_indicator, dtype=np.float32
        )
    ...
    def _load_modality(
        self, modalities: Dict[str, Path], modality_type: str
