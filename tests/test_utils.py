...
from tests.fixtures import temp_data_dir, synthetic_train_subjects, synthetic_test_subjects
...
class TestDiscoverSubjects:
    ...
    def test_discover_missing_modalities(self, synthetic_train_subjects):
        ...
        subjects_missing_phase = [
            s for s in subjects.values() 
            if "phase" not in s["modalities"]
        ]
        subjects_missing_mask = [
            s for s in subjects.values() 
            if "mask" not in s["modalities"]
        ]
    ...
    def test_stratification_maintains_balance(self):
        ...
        for fold_idx, (train_subjects, val_subjects) in enumerate(folds):
            ...
            for class_id in range(5):
                ...
                # Allow ±10% deviation
                assert abs(class_count - expected_count) <= expected_count * 0.1, \
