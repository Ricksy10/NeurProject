...
from neur.utils import set_seed, load_config, ensure_dir, discover_subjects, create_subject_folds
...
from neur.eval import generate_metrics_report
import numpy as np
...
def parse_args():
    ...
    # Optional overrides
    parser.add_argument(
        "--data-root", type=str, default=None, help="Data directory (overrides config)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["resnet18", "resnext50_32x4d", "vgg11_bn", "efficientnet_b0", "efficientnet_b1", "efficientnet_b3"],
        help="Model architecture (overrides config)",
    )
    parser.add_argument(
        "--num-folds", type=int, default=None, help="Number of CV folds (overrides config)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num-workers", type=int, default=None, help="DataLoader workers (overrides config)"
    )
...
def main():
    ...
    for fold_id, (train_subject_ids, val_subject_ids) in enumerate(folds):
        print(f"\n{'='*80}")
        print(
            f"[INFO] Fold {fold_id}/{num_folds-1}: {len(train_subject_ids)} train, {len(val_subject_ids)} val subjects"
        )
        print(f"{'='*80}")
        ...
        train_dataset = SubjectDataset(
            train_subjects, transform=train_transforms, img_size=img_size
        )
        ...
        print(
            f"[INFO] Model: {config['model']['architecture']}, "
    ...
    print(
        f"Chlorella Precision:  {np.mean(chlorella_precisions):.4f} ± {np.std(chlorella_precisions):.4f}"
    )
    print(
        f"Chlorella Recall:     {np.mean(chlorella_recalls):.4f} ± {np.std(chlorella_recalls):.4f}"
    )

    # Save aggregated metrics
    aggregated_metrics = {
        "num_folds": num_folds,
        "overall_accuracy": {"mean": float(np.mean(accuracies)), "std": float(np.std(accuracies))},
        "chlorella_f0_5": {
