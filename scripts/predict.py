...
from neur.train import load_checkpoint
...
def parse_args():
    ...
    parser.add_argument('--calibration', type=str, required=True,
                       help='Path to calibration parameters JSON')
...
def main():
    ...
    try:
        ...
        if missing_count > 0:
            print(f"[INFO] Missing modalities: {missing_count} subjects ({missing_count/len(test_subjects)*100:.1f}%)")
    except Exception as e:
    ...
    print(f"  Chlorella predicted:  {chlorella_count} ({chlorella_count/len(predictions)*100:.1f}%)")
    ...
    if validation_result['valid']:
        print(f"[SUCCESS] Submission format is valid!")
        print(f"  Header: ✓")
        print(f"  Columns: ✓")
        print(f"  Row count: {validation_result['row_count']} ✓")
        print(f"  Duplicate IDs: None ✓")
        print(f"  Target range: [0, 4] ✓")
    else:
        print(f"[ERROR] Submission validation failed!", file=sys.stderr)
