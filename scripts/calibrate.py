...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Calibrate chlorella decision threshold"
    )
    
    # Required arguments
    parser.add_argument('--val-preds', type=str, required=True,
...
def plot_threshold_sweep(
    ...
    plt.plot(recalls[optimal_idx], precisions[optimal_idx], 'ro', markersize=10,
             label=f'Optimal (τ={optimal_threshold:.3f})')
    
    # Mark target recall line
    plt.axvline(x=target_recall, color='r', linestyle='--', alpha=0.5,
...
def main():
    ...
    print(f"[INFO] Total validation predictions: {total_predictions} subjects across {len(val_predictions)} folds")
    ...
    if achieved_metrics['recall'] < args.target_recall:
        ...
        print(f"[WARN] Using closest feasible threshold. Consider retraining or adjusting target.")
    else:
    ...
    print(f"  python scripts/predict.py --test-dir data/test \\")
    print(f"    --checkpoint outputs/checkpoints/fold_0_best.pth \\")
